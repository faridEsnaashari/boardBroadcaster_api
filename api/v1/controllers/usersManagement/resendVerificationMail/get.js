...
const { INTERNAL_ERR, CONFILICT_ERR, SUCCESS_MSG, NOTFOUND_ERR } = require(`${ global.paths.tools.statusCodes }`);
...
const get = async(req, res) => {
    ...
    try{
        ...
        const token = await user.getVerificationToken({ email: user.email, verificationCode: user.verificationCode });
        await sendVerificationEmail(user.email, `${ global.env.GENERAL.APP_URL }/signup/verify_email/${ token }`, user.name);
