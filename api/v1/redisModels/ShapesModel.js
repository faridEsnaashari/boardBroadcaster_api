...
const addOrModifyShape = async(shape, room) => {
    try{
        ...
        await redisClient.set(room, JSON.stringify(shapeWasExists ? updatedShapes : [...updatedShapes, shape]));
    }
    catch(err){
        console.error(err);
    }
...
const deleteAShapeOfRoom = async(room, shape) => {
    try{
        ...
        const updatedShapes = JSON.parse(shapes).map(storedShape => storedShape.name === shape.name ? shape : storedShape);
